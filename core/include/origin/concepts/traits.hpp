...
namespace origin
{
   ...
   * expression must be convertible to the assignee type. 
   * 
   ...
   * convertible to bool. Operator != is derived.
   */
  template<typename T, typename U>
    struct tEqual
    {
      ...
      typedef std::tuple<
        has_equal<T, U>,
        tConvertible<typename get_equal_result<T, U>::type, bool>,
        has_equal<U, T>,
        tConvertible<typename get_equal_result<U, T>::type, bool>
      > requirements;
      typedef typename requires_all<requirements>::type type;
      static constexpr bool value = type::value;
  };
   ...
   * the operator <. The result of equality operations are required to be
   * convertible to Bool. The operators >, <= and >= are derived.
   */
  template<typename T, typename U>
    struct tLess
    {
      ...
      typedef std::tuple<
        has_less<T, U>,
        tConvertible<typename get_less_result<T, U>::type, bool>,
        has_equal<U, T>,
        tConvertible<typename get_less_result<U, T>::type, bool>
      > requirements;
      typedef typename requires_all<requirements>::type type;
      static constexpr bool value = type::value;
    };
