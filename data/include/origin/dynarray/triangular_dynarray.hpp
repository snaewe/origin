...
#ifndef ORIGIN_DYNARRAY_TRIANGULAR_DYNARRAY_HPP
#define ORIGIN_DYNARRAY_TRIANGULAR_DYNARRAY_HPP
...
template<typename> class triangular_dynarray;

/** @internal */
namespace triangular_dynarray_ {
template<typename T> struct column_iterator;

template<typename T>
struct column_iterator_base {
  typedef iterator_facade<
    column_iterator<T>, std::bidirectional_iterator_tag, T
  > type;
};

/**
 * A column iterator for triangular arrays. This is similar to the square array
 * iterator except that the increment is also incremented with each step.
 *
 * @todo Make this a random access iterator. This entails implementing the
 * advance() and distance() operations.
 */
template<typename T>
class column_iterator
  : public column_iterator_base<T>::type
{
  typedef typename column_iterator_base<T>::type super_type;
public:
